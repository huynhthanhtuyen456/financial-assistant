"""
Income Statement Analysis and Prediction Router.

This module defines the API endpoints for the Income Statement dashboard.
It focuses on analyzing a company's profitability through three key metrics:
Gross Profit Margin, Operating Profit Margin, and Net Profit Margin.

The module integrates Deep Learning (LSTM, RNN) and traditional Machine Learning
(Linear Regression, Random Forest) to forecast these margins based on historical data.
"""
import numpy as np
import pandas as pd
...
async def income_statement(
    ...
    """
    Generates the comprehensive Income Statement Dashboard.

    This endpoint performs the following analytical steps:
    1.  **Data Ingestion:** Fetches historical Income Statement data.
    2.  **Feature Engineering:** Calculates Gross, Operating, and Net Profit Margins.
    3.  **Correlation Analysis:** Generates a heatmap of income statement components.
    4.  **Deep Learning Prediction (LSTM & RNN):**
        -   Trains/Loads models to predict raw financial values (Revenue, Profit, etc.).
        -   Recursively forecasts future values.
        -   Derives predicted margins from forecasted raw values.
        -   Generates dynamic health assessment text.
    ...
    Args:
        ...
        feature_cols_query (list[str]): Optional list of columns to filter.
